...
import type { AboutSectionContent } from "../../services/contentService";

export const AboutSectionForm: React.FC = () => {
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");
  const [success, setSuccess] = useState(false);
  const [formData, setFormData] = useState<AboutSectionContent>({
    heading: "",
    description: "",
  });
  ...
  const fetchContent = async () => {
    try {
      ...
      setFormData(content.aboutSection);
    } catch (err: any) {
  ...
  const handleInputChange = (
    field: keyof AboutSectionContent,
    value: string,
  ) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await contentService.updateSection("aboutSection", formData);
      setSuccess(true);
