...
import type { AmenitySectionContent } from "../../services/contentService";

export const AmenitiesSectionForm: React.FC = () => {
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");
  const [success, setSuccess] = useState(false);
  const [formData, setFormData] = useState<AmenitySectionContent>({
    heading: "",
    amenities: [],
  });
  ...
  const addAmenity = () => {
    setFormData((prev) => ({
      ...prev,
      amenities: [...prev.amenities, { name: "", icon: "" }],
    }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await contentService.updateSection("amenitiesSection", formData);
      setSuccess(true);
        ...
        <div className="space-y-3">
          ...
          <div className="bg-white/70 rounded-lg p-4">
            <p className="text-gray-600 font-medium mb-2">
              Amenities ({formData.amenities?.length || 0}):
            </p>
                ...
                >
                  <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <input
                      ...
                      value={amenity.icon}
                      onChange={(e) =>
                        handleAmenityChange(index, "icon", e.target.value)
                      }
                      className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-lime-500 focus:border-transparent outline-none transition"
